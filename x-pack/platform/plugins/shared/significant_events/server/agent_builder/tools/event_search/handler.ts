...
const preventsClosure = (signal: Signal): boolean =>
  signal.verdict !== 'refutes' && signal.verdict !== 'off_topic';

const byClosureStatusThenRecency = (left: Signal, right: Signal): number => {
  if (preventsClosure(left) !== preventsClosure(right)) return preventsClosure(left) ? -1 : 1;
  ...
  signals.reduce(
    (metadata, signal) => {
      ...
      if (ruleUuid !== undefined) {
        ...
        if (preventsClosure(signal)) metadata.closureBlockingRuleUuids.add(ruleUuid);
      }
