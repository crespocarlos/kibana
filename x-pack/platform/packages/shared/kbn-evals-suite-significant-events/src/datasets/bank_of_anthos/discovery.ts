...
const toInputDetections = (discoveries: Array<Partial<Discovery>>): Array<Partial<Detection>> =>
  discoveries
    .flatMap((discovery) => (discovery.signals ?? []).filter((s) => s.metadata?.kind !== 'quiet'))
    .map((signal) => ({
 ...
 * and the judge (input). Signals carry the `evidence.esql_query` for the judge to re-run but are
 * deliberately NOT pre-stamped `confirmed` — the judge must re-verify each query via execute_esql
 * and stamp `confirmed: true` itself before promoting (Critical Rule 5). Every signal here is seeded
 * by one of the cascade detections, so the canonical input and this expected answer stay self-consistent.
 */
const LEDGER_DB_CASCADE_DISCOVERY: Partial<Discovery> = {
  ...
  blast_radius: [
    {
      type: 'dependency',
      feature_id: 'transactionhistory-ledger-db',
      source: 'transactionhistory',
      target: 'ledger-db',
    },
    {
      type: 'dependency',
      feature_id: 'balancereader-ledger-db',
    ...
    {
      type: 'dependency',
      feature_id: 'ledgerwriter-ledger-db',
      source: 'ledgerwriter',
      target: 'ledger-db',
    },
  ],
...
export const discoveryJudge: DatasetConfig['discoveryJudge'] = [
  {
    ...
    output: {
      expected_ground_truth:
        ...
        'user-blocking read timeouts)=open/critical; ' +
        'benign authentication spike (successful logins/signups only, no failures)=dismissed',
      criteria: [
        {
          id: 'open-active-cascade',
          text: 'Sets status=open with severity=critical for the cascade discovery: active database-connectivity failures cascading to user-facing transactionhistory read timeouts warrant immediate on-call action.',
          score: 3,
        },
        {
